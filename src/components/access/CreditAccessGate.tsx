...
export default function CreditAccessGate({ children, bypass = false, onGranted }: CreditAccessGateProps) {
  const navigate = useNavigate();
  const inputRef = useRef<HTMLInputElement>(null);
  const [granted, setGranted] = useState(() => bypass || hasCreditAccess());
  ...
  useEffect(() => {
    const syncAccess = () => setGranted(bypass || hasCreditAccess());
    window.addEventListener(CREDIT_ACCESS_EVENT, syncAccess);
    return () => window.removeEventListener(CREDIT_ACCESS_EVENT, syncAccess);
  }, [bypass]);
  ...
  async function submit(event: FormEvent) {
    ...
    setError('');
    try {
      if (!(await verifyCreditPassword(password))) {
  ...
  return (
    <main className="distributor-access-page" aria-labelledby="distributor-access-title">
      <section className="distributor-access-card">
        ...
        <h1 id="distributor-access-title">Crédito</h1>
